...
class BarLocation:AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(map: GoogleMap?) {
        ...
        db!!.collection("Bars").document(id).get().addOnSuccessListener { documents ->
                val status=documents.get("status").toString()
                if(status=="approved"){
                    val mMap2: GoogleMap = map!!
                    val urls: ArrayList<String> = documents.get("imagesURL") as ArrayList<String>
                    ...
                    Glide.with(this).asBitmap().load(urls[0])
                        .override(120, 120)
                        ...
                        .listener(object : RequestListener<Bitmap> {
                            ...
                            override fun onResourceReady(
                                ...
                                val iconFactory = IconGenerator(this@BarLocation)
                                ...
                                var textImage =
                                    createDrawableFromView(
                                        this@BarLocation,
                                        urls[0],
                                        documents.get("title").toString(),
                                        documents.getLong("discount")!!,
                                        documents.getLong("todayReviews")!!
                                    )
                                var scaledBitmap =
                                ...
                                var markerOptions: MarkerOptions =
                                    MarkerOptions().position(latlng).title(documents.id)
...
//            mMap.animateCamera(CameraUpdateFactory.newCameraPosition(cameraPosition))
    ...
    private fun createDrawableFromView(
        ...
        val view: View = this@BarLocation.layoutInflater.inflate(R.layout.example, null)
...
//        Log.d("Anas", "${view.measuredWidth}==width    ${view.measuredHeight}==height")
    ...
    private fun markerClicked(id: String) {
        val displayMetrics = DisplayMetrics()
        this@BarLocation.windowManager.defaultDisplay.getMetrics(displayMetrics)
        val height = displayMetrics.heightPixels
        val width = displayMetrics.widthPixels
        val density=displayMetrics.densityDpi
        var dialog: Dialog = Dialog(this@BarLocation)
